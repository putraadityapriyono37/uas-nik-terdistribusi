<?php
$app = require __DIR__ . '/../app/config/app.php';

$currentPath = getRequestPath();

$page = 'dashboard.php';
$pageTitle = 'Dashboard';

if ($currentPath === '/citizens') {
    $page = 'citizens.php';
    $pageTitle = 'Data Warga';
}

if ($currentPath === '/medical-records') {
    $page = 'medical_records.php';
    $pageTitle = 'Rekam Medis';
}

if ($currentPath === '/audit-logs') {
    $page = 'audit_logs.php';
    $pageTitle = 'Audit Log';
}

function isActiveMenu($path, $currentPath)
{
    if ($path === '/' && $currentPath === '/') {
        return true;
    }

    return $path !== '/' && strpos($currentPath, $path) === 0;
}
?>

<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= htmlspecialchars($pageTitle) ?> - <?= htmlspecialchars($app['name']) ?></title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>

<body class="bg-slate-50 text-slate-800">
    <div class="min-h-screen">

        <!-- Mobile Header -->
        <header class="sticky top-0 z-40 border-b border-slate-200 bg-white px-4 py-3 md:hidden">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                        Layanan NIK
                    </p>
                    <h1 class="text-base font-semibold text-slate-900">
                        E-KTP Service
                    </h1>
                </div>

                <button
                    type="button"
                    onclick="toggleMobileMenu()"
                    class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-slate-700"
                    aria-label="Buka menu"
                >
                    <span class="text-lg leading-none">☰</span>
                </button>
            </div>
        </header>

        <!-- Mobile Menu -->
        <div id="mobileMenu" class="hidden border-b border-slate-200 bg-slate-950 px-4 py-4 md:hidden">
            <nav class="space-y-1">
                <a href="/"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Dashboard
                </a>

                <a href="/citizens"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/citizens', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Data Warga
                </a>

                <a href="/medical-records"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/medical-records', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Rekam Medis
                </a>

                <a href="/audit-logs"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/audit-logs', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Audit Log
                </a>
            </nav>
        </div>

        <!-- Desktop Sidebar -->
        <aside class="fixed inset-y-0 left-0 hidden w-64 border-r border-slate-200 bg-slate-950 px-6 py-6 md:block">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                    Layanan NIK
                </p>
                <h1 class="mt-2 text-lg font-semibold text-white">
                    E-KTP Service
                </h1>
                <p class="mt-1 text-xs leading-5 text-slate-400">
                    Pusat verifikasi identitas warga.
                </p>
            </div>

            <nav class="mt-8 space-y-1">
                <a href="/"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Dashboard
                </a>

                <a href="/citizens"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/citizens', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Data Warga
                </a>

                <a href="/medical-records"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/medical-records', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Rekam Medis
                </a>

                <a href="/audit-logs"
                   class="block rounded-lg px-3 py-2 text-sm font-medium <?= isActiveMenu('/audit-logs', $currentPath) ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
                    Audit Log
                </a>
            </nav>

            <div class="absolute bottom-6 left-6 right-6 rounded-lg border border-slate-700 bg-slate-900 p-4">
                <p class="text-xs font-medium text-slate-400">Port Service</p>
                <p class="mt-1 font-mono text-sm text-white">localhost:8000</p>
            </div>
        </aside>

        <!-- Main Content -->
        <main class="md:pl-64">
            <header class="hidden border-b border-slate-200 bg-white px-6 py-4 md:block md:px-8">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-slate-500">Sistem Integrasi Layanan Publik</p>
                        <p class="text-base font-semibold text-slate-900">
                            <?= htmlspecialchars($pageTitle) ?>
                        </p>
                    </div>

                    <div class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">
                        Service Aktif
                    </div>
                </div>
            </header>

            <div class="px-4 py-6 md:px-8 md:py-8">
                <?php require __DIR__ . '/' . $page; ?>
            </div>
        </main>
    </div>

    <script>
        function toggleMobileMenu() {
            const menu = document.getElementById('mobileMenu');
            menu.classList.toggle('hidden');
        }
    </script>
</body>
</html>